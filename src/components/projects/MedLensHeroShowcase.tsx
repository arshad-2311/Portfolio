...
export const MedLensHeroShowcase: React.FC<MedLensHeroShowcaseProps> = ({
  ...
  const steps = [
    { label: '01. SCAN / IMAGE', icon: Scan, desc: 'High-res input capture of blister packaging' },
    { label: '02. OCR & VISION', icon: Sparkles, desc: 'Gemini 2.5 Flash detects entity boundaries' },
    { label: '03. STRUCTURED DATA', icon: CheckCircle2, desc: 'Deterministic JSON entity extraction' },
    { label: '04. VERNACULAR TTS', icon: Volume2, desc: 'Multi-dialect translation & ElevenLabs audio' },
  ];

  return (
    <div id="project-medlens" className="border border-white/[0.12] bg-[#0D0D0D] rounded-lg p-6 sm:p-10 transition-all hover:border-white/20">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-white/[0.08]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="mono-label text-[11px] text-[#3B82F6] font-semibold">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-1.5 rounded bg-[#3B82F6] hover:bg-[#2563EB] text-xs font-mono text-white font-medium flex items-center gap-1.5 transition-colors shadow-sm"
      ...
      <div className="py-6 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-7 space-y-4">
          <p className="text-base text-[#D4D4D8] leading-relaxed font-light">
            {project.tagline}
          </p>
          ...
          <div className="flex flex-wrap gap-1.5 pt-2">
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="px-2 py-0.5 rounded bg-[#161616] text-[#A1A1AA] text-[11px] font-mono border border-white/[0.06]"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="lg:col-span-5 bg-[#121212] border border-white/[0.08] rounded p-4 space-y-2.5">
          ...
          <div className="text-xs font-mono text-[#A1A1AA] space-y-1.5">
      ...
      <div className="mt-4 border border-white/[0.08] rounded-md bg-[#0A0A0A] p-5 space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-white/[0.06]">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-[#3B82F6] animate-pulse" />
            <span className="mono-label text-xs text-[#F5F5F5]">
              MULTIMODAL PIPELINE EVALUATION SANDBOX
            </span>
          </div>
          <span className="text-[10px] font-mono text-[#71717A] bg-[#141414] px-2 py-0.5 rounded border border-white/[0.06]">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <Icon className={`w-3.5 h-3.5 ${isCur ? 'text-[#3B82F6]' : 'text-[#52525B]'}`} />
                  <span className="text-xs font-mono font-medium">{st.label}</span>
                </div>
                <div className="text-[10px] truncate text-[#71717A]">{st.desc}</div>
              </button>
            );
          })}
        </div>
        ...
        <div className="bg-[#111111] border border-white/[0.08] rounded p-4 font-mono text-xs">
          {activeStep === 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between text-[#71717A] border-b border-white/[0.04] pb-2">
                <span>INPUT_IMAGE // AUGMENT_500MG.JPG</span>
                <span>STATUS: LOADED</span>
              </div>
              <div className="p-3 bg-[#0D0D0D] border border-dashed border-white/10 rounded flex items-center justify-between">
                <div className="space-y-1">
                  <div className="text-white font-medium">Augmentin 625 Duo (Amoxicillin + Potassium Clavulanate)</div>
                  <div className="text-[11px] text-[#71717A]">Blister Strip · 10 Tablets · Exp: 11/2027 · Batch: #AUG-8821</div>
                </div>
                <button
                  onClick={() => setActiveStep(1)}
                  className="px-3 py-1 bg-white/10 hover:bg-white/20 text-white rounded text-[11px]"
                >
                  Run OCR →
                </button>
              </div>
          ...
          {activeStep === 1 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between text-[#71717A] border-b border-white/[0.04] pb-2">
                <span>GEMINI 2.5 FLASH // BOUNDING BOX OCR INFERENCE</span>
                <span className="text-[#10B981]">CONFIDENCE: 98.4%</span>
              </div>
              <div className="bg-[#0D0D0D] p-3 rounded space-y-2 text-[11px]">
          ...
          {activeStep === 2 && (
            <div className="space-y-3">
              ...
              <pre className="bg-[#0D0D0D] p-3 rounded text-[11px] text-[#D4D4D8] overflow-x-auto">
{`{
  "brand_name": "Augmentin 625 Duo",
  "active_molecules": ["Amoxicillin (500mg)", "Clavulanic Acid (125mg)"],
  "pharmacological_class": "Penicillin-class Antibiotic + Beta-lactamase inhibitor",
  ...
  "schedule_type": "Schedule H1 Prescription"
          ...
          {activeStep === 3 && (
            <div className="space-y-4">
              ...
              {/* Translation Content */}
              <div className="bg-[#0D0D0D] p-4 rounded space-y-2.5">
                ...
                <div className="pt-3 border-t border-white/[0.06] flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="p-1.5 rounded-full bg-white text-black hover:bg-[#E4E4E7] transition-colors"
